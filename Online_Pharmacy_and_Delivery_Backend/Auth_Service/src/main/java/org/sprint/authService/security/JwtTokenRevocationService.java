package org.sprint.authService.security;
...
public class JwtTokenRevocationService {
    ...
    public void revokeToken(String tokenId, String rawToken, Instant expiresAt) {
        ...
        if (redisTemplate == null) {
            log.warn("JWT revocation is enabled but Redis is unavailable. Token revocation skipped.");
            return;
        }
        ...
        try {
            if (tokenId != null && !tokenId.isBlank()) {
                redisTemplate.opsForValue().set(jtiPrefix + tokenId.trim(), "1", ttl);
            }

            String fingerprint = fingerprint(rawToken);
            if (fingerprint != null) {
                redisTemplate.opsForValue().set(hashPrefix + fingerprint, "1", ttl);
            }
        } catch (Exception ex) {
            log.warn("Failed to persist token revocation marker: {}", ex.getMessage());
        }
    }
